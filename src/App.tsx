          ...
          <motion.div key="timeline" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
            ...
            {(() => {
              ...
              return allItems.map(item => item.type === 'drink' ? (
                <motion.div key={item.data.id} layout initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="glass p-4 rounded-2xl flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => deleteLog(item.data.id)}
                      disabled={(item.data as any)._isPending}
                      className="w-7 h-7 rounded-full flex items-center justify-center text-stone-300 hover:text-red-400 hover:bg-red-50 active:text-red-400 active:bg-red-50 transition-all disabled:opacity-30 disabled:pointer-events-none"
                    >
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex-1">
                  <input
                    ...
                    onChange={e => setUserProfile(p => ({...p, username: e.target.value}))}
                    className="text-xl font-bold bg-transparent outline-none border-b-2 border-transparent focus:border-neon-blue transition-colors w-full leading-tight"
                    placeholder="Your name"
                  />
                  <p className="text-stone-400 text-sm">{session.user.email}</p>
                </div>
              </div>
              <AnimatePresence>
                {showAvatarPicker && (
